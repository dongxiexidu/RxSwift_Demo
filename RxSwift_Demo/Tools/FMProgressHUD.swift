...
//MARK: - FMProgressHUDDelegate
@objc protocol FMProgressHUDDelegate {
    @objc optional func hudWasHidden(_ hud: FMProgressHUD)
}

//MARK: - ENUM
enum FMProgressHUDMode: Int {
...
enum FMProgressHUDAnimation: Int {
    case fade = 0
    case zoom
    case zoomOut
    case zoomIn
...
typealias FMProgressHUDCompletionBlock = () -> Void
typealias FMProgressHUDExecutionClosures = () -> Void
...
class FMProgressHUD: UIView {
    ...
    fileprivate var closureForExecution: FMProgressHUDExecutionClosures?
    ...
    var animationType = FMProgressHUDAnimation.fade
    var mode = FMProgressHUDMode.indeterminate {
        didSet {
            self.updateIndicators()
            self.updateUI()
        }
    ...
    var completionBlock: FMProgressHUDCompletionBlock?
    
    var delegate: FMProgressHUDDelegate?
    ...
    func show(_ animated: Bool) {
        assert(Thread.isMainThread, "FMProgressHUD needs to be accessed on the main thread.")
...
//        isFinished = true
    ...
    // MARK: - Threading
    func showWhileExecuting(_ closures: @escaping FMProgressHUDExecutionClosures, animated: Bool) {
    ...
    func showAnimated(_ animated: Bool, whileExecutingBlock block: @escaping ()->(), completionBlock completion: FMProgressHUDCompletionBlock?) {
    ...
    func showAnimated(_ animated: Bool, whileExecutingBlock block: @escaping ()->(), onQueue queue: DispatchQueue, completionBlock completion: FMProgressHUDCompletionBlock?) {
        taskInprogress = true
    ...
    fileprivate func updateIndicators() {
        ...
        let isRoundIndicator: Bool = self.indicator is FMRoundProgressView
        let isIndeterminatedRoundIndicator: Bool = self.indicator is FMIndeterminatedRoundProgressView
        ...
        case .annularIndeterminate:
            if !isIndeterminatedRoundIndicator {
                self.indicator?.removeFromSuperview()
                self.indicator = FMIndeterminatedRoundProgressView()
                self.addSubview(self.indicator!)
            }
            
        case .determinateHorizontalBar:
            self.indicator?.removeFromSuperview()
            self.indicator = FMBarProgressView()
            self.addSubview(self.indicator!)
        ...
        case .annularDeterminate:
            if !isRoundIndicator {
                self.indicator?.removeFromSuperview()
                self.indicator = FMRoundProgressView()
                self.addSubview(self.indicator!)
            }
            
            if self.mode == FMProgressHUDMode.annularDeterminate {
                (self.indicator as! FMRoundProgressView).annular = true
            }
...
//        if mode == FMProgressHUDMode.Indeterminate {
//            if !isActivityIndicator {
...
//        } else if mode == FMProgressHUDMode.AnnularIndeterminate {
//            if !isIndeterminatedRoundIndicator {
//                indicator?.removeFromSuperview()
//                indicator = MBIndeterminatedRoundProgressView()
//                self.addSubview(indicator!)
//            }
//        } else if mode == FMProgressHUDMode.DeterminateHorizontalBar {
//            indicator?.removeFromSuperview()
//            indicator = MBBarProgressView()
//            self.addSubview(indicator!)
//        } else if mode == FMProgressHUDMode.Determinate || mode == FMProgressHUDMode.AnnularDeterminate {
//            if !isRoundIndicator {
//                indicator?.removeFromSuperview()
//                indicator = MBRoundProgressView()
//                self.addSubview(indicator!)
//            }
//            if mode == FMProgressHUDMode.AnnularDeterminate {
//                (indicator as! MBRoundProgressView).annular = true
//            }
//        } else if mode == FMProgressHUDMode.CustomView && customView != indicator {
//            indicator?.removeFromSuperview()
//            self.indicator = customView
//            self.addSubview(indicator!)
//        } else if mode == FMProgressHUDMode.Text {
//            indicator?.removeFromSuperview()
//            indicator = nil
//        }
    }
...
// MARK: - Class methods
extension FMProgressHUD {
    
    class func showHUDAddedTo(_ view: UIView, animated: Bool) -> FMProgressHUD {
        let hud: FMProgressHUD = FMProgressHUD(view: view)
    ...
    class func showLoadingHUDAddedTo(_ view: UIView, animated: Bool){
        let hud: FMProgressHUD = FMProgressHUD(view: view)
        hud.removeFromSuperViewOnHide = true
        view.addSubview(hud)
        hud.show(animated)
...
//        return hud
    ...
    class func hideAllHUDsForView(_ view: UIView, animated: Bool) -> Int {
        let huds = FMProgressHUD.allHUDsForView(view)
        for hud in huds {
            hud.removeFromSuperViewOnHide = true
            hud.hide(animated)
        }
    ...
    class func hideAllLoadingHUDsForView(_ view: UIView, animated: Bool) {
        let huds = FMProgressHUD.allHUDsForView(view)
        for hud in huds {
            hud.removeFromSuperViewOnHide = true
            hud.hide(animated)
...
//        return huds.count
    ...
    class func HUDForView(_ view: UIView) -> FMProgressHUD? {
        for subview in Array(view.subviews.reversed()) {
            if subview is FMProgressHUD {
                return subview as? FMProgressHUD
            }
        }
    ...
    class func allHUDsForView(_ view: UIView) -> [FMProgressHUD] {
        var huds: [FMProgressHUD] = []
        for aView in view.subviews {
            if aView is FMProgressHUD {
                huds.append(aView as! FMProgressHUD)
            }
        }
...
// MARK: - FMRoundProgressView
class FMRoundProgressView: UIView {
    var progress: Float = 0.0 {
        didSet {
            self.updateUI()
        }
...
class FMBarProgressView: UIView {
    var progress: Float {
        didSet {
            self.updateUI()
        }
...
class FMIndeterminatedRoundProgressView: UIView {
